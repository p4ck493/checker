...
describe('Object singel: apply', () => {
    ...
    it.each(dataForGoodTesting)('!is.compare: Should false for $first === $second', ({first, second}) => {
        expect(!is.compare.apply({}, [first, second] as any)).toBe(false);
    });

    it.each(dataForBadTesting)('!is.compare: Should true for $first === $second', ({first, second}) => {
        expect(!is.compare.apply({}, [first, second] as any)).toBe(true);
    });
