...
describe('array singel: apply', () => {
    ...
    it.each(dataForGoodTesting)('!is.array: Should false for $value', ({value}) => {
        expect(!is.array.apply({}, [value])).toBe(false);
    });

    it.each(dataForBadTesting)('!is.array: Should true for $value', ({value}) => {
        expect(!is.array.apply({}, [value])).toBe(true);
    });
    ...
    it.each(dataForGoodAdvancedTesting)('!is.array: Should false for $value & $classRef', ({value, classRef}) => {
        expect(!is.array(value, classRef)).toBe(false);
    });
